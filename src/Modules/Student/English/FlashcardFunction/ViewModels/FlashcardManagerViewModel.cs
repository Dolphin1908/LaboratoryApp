...
namespace LaboratoryApp.src.Modules.Student.English.FlashcardFunction.ViewModels
{
    public class FlashcardManagerViewModel : BaseViewModel
    {
        ...
        private readonly Func<IDialogService, ISpeechService, FlashcardSet, IFlashcardService, FlashcardStudyViewModel> _flashcardStudyVmFactory; // Factory để tạo FlashcardStudyViewModel cho việc ôn tập thẻ flashcard
        ...
        public FlashcardManagerViewModel(IFlashcardService flashcardService,
                                         ...
                                         Func<IDialogService, ISpeechService, FlashcardSet, IFlashcardService, FlashcardStudyViewModel> flashcardStudyVmFactory,
                                         Func<DictionaryWindow> dictionaryWindowFactory)
        {
            ...
            OpenUpdateFlashcardWindowCommand = new RelayCommand<object>((p) => true, (p) =>
            {
                ...
                if (flashcard == null)
                {
                    _dialogService.ShowMessage("Thẻ flashcard không tồn tại.", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            ...
            StartFlashcardSetCommand = new RelayCommand<object>((p) => true, (p) =>
            {
                ...
                {
                    if (_dialogService.ShowMessage($"Bộ thẻ '{SelectedFlashcardSet.Name}' không có thẻ nào cần ôn tập!", "Bạn có muốn tiếp tục học?", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.No)
                        return;
                }
        ...
        {
            if (_dialogService.ShowMessage($"Bạn có chắc muốn xóa {setToDelete.Name}?", "Cảnh báo", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
